...
def render():
    ...
    # ─── 2) In-page filters (dynamic 3×2 grid, no Mean Income) ────────────────
    # compute each slider’s min/max from the data
    ...
    # Row 1: Median Income, Private Schools, Vessel Count
    ...
    # Row 2: Homes >$1M, Home Value Growth, Divorce Rate
    ...
    # recompute rank so 1 = highest wealth
    ...
    with st.container():
        ...
        with table_col:
            ...
            topn.columns = ["Rank","ZIP","Area","Median Income",
    ...
    for i,col in enumerate(zip_cols):
        with col:
            z = st.selectbox(f"ZIP {i+1}", opts, index=min(i, len(opts)-1), key=f"zip{i}")
    ...
    with rcol:
        ...
        fig_r = px.line_polar(radar_df, r="Value", theta="Metric",
